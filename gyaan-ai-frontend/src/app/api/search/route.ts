...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!query) {
      return NextResponse.json({ error: 'Query parameter is required' }, { status: 400, headers: { 'Cache-Control': 'no-store, must-revalidate', 'CDN-Cache-Control': 'no-store, must-revalidate' } });
    }
    ...
    if (mode === 'images') {
      ...
      if (!unsplashAccessKey) {
        return NextResponse.json({ error: 'Unsplash API key is not configured' }, { status: 500, headers: { 'Cache-Control': 'no-store, must-revalidate', 'CDN-Cache-Control': 'no-store, must-revalidate' } });
      }
      ...
      if (!newsApiKey) {
        return NextResponse.json({ error: 'NewsAPI key is not configured' }, { status: 500, headers: { 'Cache-Control': 'no-store, must-revalidate', 'CDN-Cache-Control': 'no-store, must-revalidate' } });
      }
      ...
      if (!youtubeApiKey) {
        return NextResponse.json({ error: 'YouTube API key is not configured' }, { status: 500, headers: { 'Cache-Control': 'no-store, must-revalidate', 'CDN-Cache-Control': 'no-store, must-revalidate' } });
      }
      ...
      if (googleApiKey && googleSearchEngineId) {
        ...
        if (results.length === 0) {
          const newsApiKey = process.env.NEWSAPI_KEY;
          if (!newsApiKey) {
            return NextResponse.json({ error: 'No search API keys configured (Google Search, DuckDuckGo, or NewsAPI)' }, { status: 500, headers: { 'Cache-Control': 'no-store, must-revalidate', 'CDN-Cache-Control': 'no-store, must-revalidate' } });
          }
      ...
      return NextResponse.json({ error: 'Invalid mode. Use "news", "trending", "web", "images", or "videos"' }, { status: 400, headers: { 'Cache-Control': 'no-store, must-revalidate', 'CDN-Cache-Control': 'no-store, must-revalidate' } });
    }

    return NextResponse.json({ results }, { headers: { 'Cache-Control': 'no-store, must-revalidate', 'CDN-Cache-Control': 'no-store, must-revalidate' } });
  } catch (error) {
    console.error('Search API error:', error);
    return NextResponse.json({ error: 'Internal server error', message: error instanceof Error ? error.message : 'Unknown error' }, { status: 500, headers: { 'Cache-Control': 'no-store, must-revalidate', 'CDN-Cache-Control': 'no-store, must-revalidate' } });
  }
}
