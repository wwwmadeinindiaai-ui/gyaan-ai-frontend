...
interface SearchResult {
  ...
  source: string;
  publishedAt: string;
  // Unsplash attribution fields (optional for non-image modes)
  attribution?: {
...
// DuckDuckGo Instant Answer result types
...
export async function GET(request: NextRequest) {
  try {
    ...
    // News and trending modes with NewsAPI
    ...
    // Web mode
      ...
      if (googleApiKey && googleSearchEngineId) {
        ...
        results = (data.items || []).map(
          (item: GoogleSearchItem, index: number) => {
        ...
        try {
          ...
          if (ddgResponse.ok) {
            ...
            if (ddgData.RelatedTopics && Array.isArray(ddgData.RelatedTopics)) {
              ddgData.RelatedTopics.forEach(
                (topic: DuckDuckGoRelatedTopic | { Topics?: DuckDuckGoRelatedTopic[] }, index: number) => {
