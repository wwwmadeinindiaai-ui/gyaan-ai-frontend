...
export default function DashboardPage() {
  ...
  const handleSearch = () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to synthesize query');
      }

      const data = await response.json();
      setResults(data);
      setMessage(`Showing ${data.data.length} synthesized results for: "${query}"`);
    } catch (error) {
      console.error('Error synthesizing query:', error);
      setMessage('Error processing your query. Please try again.');
      setResults(null);
    } finally {
      setLoading(false);
    }
